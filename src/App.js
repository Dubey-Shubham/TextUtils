...
function App() {
  const [mode, setMode] = useState('light');       //whetrher dark mode is enabled or not iski state bana di aur state ko switch kr payenge dark light mode k beech me
  ...
  return (
    ...
    <Navbar title="TextUtils211" aboutText="About site" mode={mode} toggleMode={toggleMode} /> {/*yahan navbar components me dal rha hoon, aur yahan prop use kara hai title wala*/} 
      ...
      {/* <Routes> */}
          {/* <Route exact path='/about' element={<About/>}> </Route>             Routing k liye enclose kar diya about aur text form ko, iske alawa exact match karna hai to avoid partiall matching */}
